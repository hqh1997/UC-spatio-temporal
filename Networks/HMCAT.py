...
class HMCAT(nn.Module):
    ...
    def forward(self, wsi, macros, texts):

        H_bag = self.H(wsi).squeeze(0)
        M_bag = self.G[0](macros.type(torch.float32)).squeeze(0)
        T_bag = self.G[1](texts.type(torch.float32)).squeeze(0)
        ...
        h = self.fusion_layer(h_path_M, h_macro, h_path_T, h_text)
        # print(h.shape)
        ...
        attention_scores = {'M_coattn': M_coattn,  'T_coattn': T_coattn,  'macro':A_macro, 'text':A_text,
                             'A_path_M':A_path_M, 'A_path_T':A_path_T}  
